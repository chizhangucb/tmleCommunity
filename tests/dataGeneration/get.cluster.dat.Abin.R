...
get.fullDat.Abin <- function(J, n.ind, rndseed = NULL, is.Y.bin = TRUE, working.model = TRUE, 
  ...
  for(j in 1:J) {
    ...
    if (onlyYkeep) {
      ...
      Y1 <- c(Y.gstar, cluster.data.j[, "Y1"])
      Y0 <- c(Y.gstar, cluster.data.j[, "Y0"])
    } else {
...
comPop.wmT.bA.bY <- get.fullDat.Abin(J = 4000, n.ind = 4000, rndseed = NULL, is.Y.bin = T, working.model = T, n.ind.fix = T)
mean(comPop.wmT.bA.bY$Y1) # 
mean(comPop.wmT.bA.bY$Y0) # 
psi0.Y <- mean(comPop.wmT.bA.bY$Y1) - mean(comPop.wmT.bA.bY$Y0) # 
