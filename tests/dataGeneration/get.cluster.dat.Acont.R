...
get.cluster.dat.Acont <- function(id, n.ind = 1000, rndseed = NULL, is.Y.bin = T, truncBD = 5, shift.val = 1, 
  ...
  if (working.model) {  # when working.model holds
    if (is.Y.bin) {
      Y <- rbinom(n = n.ind, size = 1, prob = plogis(- 1.7 + 0.6 * A + 0.5 * E1 - 1.2 * E2 + 0.7 * W1 + 1.3 * W2 - 0.4 * W3))
      Y.gstar <- rbinom(n = n.ind, size = 1, prob = plogis(- 1.7 + 0.6 * trunc.A.gstar + 0.5 * E1 - 1.2 * E2 + 0.7 * W1 + 1.3 * W2 - 0.4 * W3))
    } else {
      # finishing ....
    }
  } else {  # when working.model fails
    if (is.Y.bin) {
      Y <- rbinom(n = n.ind, size = 1, prob = plogis(- 5 + 1 * A - 0.2 * E1 + 1.1 * E2 - 0.4 * W1 + 0.2 * W2 - 0.4 * W3 + 
