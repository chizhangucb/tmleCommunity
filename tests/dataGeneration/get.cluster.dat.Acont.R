...
get.cluster.dat.Acont <- function(id, n.ind = 1000, rndseed = NULL, is.Y.bin = T, truncBD = 5, shift.val = 1, 
  ...
  r.new.A <- exp(2.8 * shift.val * (untrunc.A.gstar - A.mu - shift.val / 10))
  trunc.A.gstar <- ifelse(r.new.A > truncBD, A, untrunc.A.gstar)
  
  # Construct outcome
  if (working.model) {  # when working.model holds
    if (is.Y.bin) {
      Y <- rbinom(n = n.ind, size = 1, prob = plogis(- 1.7 + 3.4 * A + 0.5 * E1 - 1.2 * E2 + 0.8 * W1 + 1 * W2 - 0.4 * W3))
      Y.gstar <- rbinom(n = n.ind, size = 1, prob = plogis(- 1.7 + 3.4 * trunc.A.gstar + 0.5 * E1 - 1.2 * E2 + 0.8 * W1 + 1 * W2 - 0.4 * W3))
    } else {
      # finishing ....
    }
