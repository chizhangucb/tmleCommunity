# ---------------------------------------------------------------------------------
# DATA SET 1. TESTS FOR FITTING CONTINUOUS EXPOSURE A IN IID DATA
# ---------------------------------------------------------------------------------
# Fitting continuous exposure by binning, conditional on covariates
# individual exposure is normal with mu for each observation being a function of (W1, W2, W3, W4), sd = 1;
# ---------------------------------------------------------------------------------

################################################
#### FUN 1. One A with Binary Y
################################################
get.iid.dat.Acont1 <- function(ndata = 100000, rndseed = NULL, truncBD = 10, shift.val = 2) {
  require(simcausal)
  D <- DAG.empty()
  D <- D + 
    node("W1", distr = "rbern", prob = 0.5) +
    node("W2", distr = "rbern", prob = 0.3) +
    node("W3", distr = "rnorm", mean = 0, sd = 0.25) +
    node("W4", distr = "runif", min = 0, max = 1) +
    node("A.mu", distr = "rconst", const = (0.86 * W1 + 0.41 * W2 - 0.34 * W3 + 0.93 * W4)) +
    node("A", distr = "rnorm", mean = A.mu, sd = 1) +
    node("Y", distr = "rbern", prob = plogis(-0.22 + 0.12 * A - 0.92 * W1 - 0.36 * W2 + 0.12 * W3 - 0.53 * W4)) +
    node("shift",  distr = "rconst", const = .(shift.val)) +
    node("untrunc.A.gstar",  distr = "rconst", const = A + shift) +
    node("r.new.A",  distr = "rconst", const = exp(0.8 * shift * (untrunc.A.gstar - A.mu - shift / 3))) +
    node("trunc.c",  distr = "rconst", const = .(truncBD)) +
    node("trunc.A.gstar",  distr = "rconst", const = ifelse(r.new.A > trunc.c, A, untrunc.A.gstar)) +
    node("Y.gstar", distr = "rbern", prob = plogis(-0.22 + 0.12 *trunc.A.gstar - 0.92 * W1 - 0.36 * W2 + 0.12 * W3 - 0.53 * W4))
  
  D <- set.DAG(D)
  Odata <- sim(D, n = ndata, rndseed = rndseed)
  # head(Odata, 50)
  psi0.Y <- mean(Odata$Y)
  print("psi0.Y: " %+% psi0.Y)  
  psi0.Ygstar <- mean(Odata$Y.gstar)
  print("psi0.Ygstar: " %+% psi0.Ygstar)
  
  return(list(psi0.Y = psi0.Y, psi0.Ygstar = psi0.Ygstar, Odata = Odata))
}

ndata <- 10000
rndseed <- 12345
truncBD <- 10
shift.val <- 1
popDat <- get.iid.dat.Acont1(ndata = 1000000, rndseed = rndseed, truncBD = truncBD, shift.val = shift.val)$Odata
psi0.Y <- mean(popDat$Y)  # 0.291398
psi0.Ygstar <- mean(popDat$Y.gstar)  # 0.316274

dat_iidcontABinY <- get.iid.dat.Acont1(ndata = ndata, rndseed = rndseed, truncBD = truncBD, shift.val = shift.val)$Odata
dat_iidcontABinY <- dat_iidcontABinY[, c("W1", "W2", "W3", "W4", "A", "Y", "Y.gstar", "trunc.A.gstar")]
sampleDat_iidcontABinY <- list(dat_iidcontABinY = dat_iidcontABinY, psi0.Y = psi0.Y, psi0.Ygstar = psi0.Ygstar,
                               truncBD = truncBD, shift.val = shift.val, rndseed = rndseed,
                               call = "get.iid.dat.Acont1(ndata = 10000, rndseed = 12345, truncBD = 10, shift.val = 1)$Odata")
save(sampleDat_iidcontABinY, file="sampleDat_iidcontABinY.Rda")


################################################
#### FUN 2. One A with Continuous Y
################################################
get.iid.dat.Acont2 <- function(ndata = 100000, rndseed = NULL, truncBD = 10, shift.val = 2) {
  require(simcausal)
  D <- DAG.empty()
  D <- D + 
    node("W1", distr = "rbern", prob = 0.5) +
    node("W2", distr = "rbern", prob = 0.3) +
    node("W3", distr = "rnorm", mean = 0, sd = 0.25) +
    node("W4", distr = "runif", min = 0, max = 1) +
    node("A.mu", distr = "rconst", const = (0.86 * W1 + 0.41 * W2 - 0.34 * W3 + 0.93 * W4)) +
    node("A", distr = "rnorm", mean = A.mu, sd = 1) +
    node("Y", distr = "rnorm", mean = (3.63 + 0.11 * A - 0.52 * W1 - 0.36 * W2 + 0.12 * W3 - 0.13 * W4), sd = 1) +
    node("shift",  distr = "rconst", const = .(shift.val)) +
    node("untrunc.A.gstar",  distr = "rconst", const = A + shift) +
    node("r.new.A",  distr = "rconst", const = exp(0.5 * shift * (untrunc.A.gstar - A.mu - shift / 2))) +
    node("trunc.c",  distr = "rconst", const = .(truncBD)) +
    node("trunc.A.gstar",  distr = "rconst", const = ifelse(r.new.A > trunc.c, A, untrunc.A.gstar)) +
    node("Y.gstar", distr = "rnorm", mean = (3.63 + 0.11 * trunc.A.gstar - 0.52 * W1 - 0.36 * W2 + 0.12 * W3 - 0.13 * W4), sd = 1)
  
  D <- set.DAG(D)
  Odata <- sim(D, n = ndata, rndseed = rndseed)
  # head(Odata, 50)
  psi0.Y <- mean(Odata$Y)
  print("psi0.Y: " %+% psi0.Y)  
  psi0.Ygstar <- mean(Odata$Y.gstar)
  print("psi0.Ygstar: " %+% psi0.Ygstar)
  
  return(list(psi0.Y = psi0.Y, psi0.Ygstar = psi0.Ygstar, Odata = Odata))
}

ndata <- 10000
rndseed <- 12345
truncBD <- 10
shift.val <- 2
popDat <- get.iid.dat.Acont2(ndata = 1000000, rndseed = rndseed, truncBD = truncBD, shift.val = shift.val)$Odata
psi0.Y <- mean(popDat$Y)  # 3.309084
psi0.Ygstar <- mean(popDat$Y.gstar)  # 3.50856

dat_iidcontAContY <- get.iid.dat.Acont2(ndata = ndata, rndseed = rndseed, truncBD = truncBD, shift.val = shift.val)$Odata
dat_iidcontAContY <- dat_iidcontAContY[, c("W1", "W2", "W3", "W4", "A", "Y", "Y.gstar", "trunc.A.gstar")]
sampleDat_iidcontAContY <- list(dat_iidcontAContY = dat_iidcontAContY, psi0.Y = psi0.Y, psi0.Ygstar = psi0.Ygstar,
                                truncBD = truncBD, shift.val = shift.val, rndseed = rndseed,
                                call = "get.iid.dat.Acont2(ndata = 10000, rndseed = 12345, truncBD = 10, shift.val = 2)$Odata")
save(sampleDat_iidcontAContY, file="sampleDat_iidcontAContY.Rda")
