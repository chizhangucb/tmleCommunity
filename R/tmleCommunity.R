...
tmleCommunity <- function(data, Ynode, Anodes, WEnodes, YnodeDet = NULL, communityID = NULL, working.model = FALSE,
  ...
  #----------------------------------------------------------------------------------
  EY_gstar1 <- calcParameters(inputYs = inputYs, alpha = CI_alpha, est_params_list = estinfo_list_g1, tmle_g_out = tmle_gstar1_out)
  EY_gstar2 <- NULL
  ATE <- NULL	
  otherInfo2 <- NULL
  if (!is.null(f_gstar2)) {
    EY_gstar2 <- calcParameters(inputYs = inputYs, alpha = CI_alpha, est_params_list = estinfo_list_g1, tmle_g_out = tmle_gstar2_out)
    ATE <- calcParameters(inputYs = inputYs, alpha = CI_alpha, est_params_list = estinfo_list_g1, tmle_g_out = tmle_gstar1_out, tmle_g2_out = tmle_gstar2_out)
  }
  message("######################################################################################")
